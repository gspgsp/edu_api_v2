...
func BoutiqueList(ctx *fasthttp.RequestCtx) {
	...
	rows, err := connect.Db.Queryx("select id, title, subtitle, difficulty_level, learn_num, buy_num, price, discount, discount_end_at, vip_level, vip_price, cover_picture from h_edu_courses where status = 'published' order by updated_at desc, sort desc limit 5")
